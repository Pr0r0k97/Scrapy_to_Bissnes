...
def get_html(url, retry=5):  # Делаем запрос к странице
    try:
        ...
        print(f'Выполняется парсинг РЕмонь данной страницы {url}')
...
def get_page_data(html):
    global mail_2, number, city, operator, number_one
    hrefs_data = []
    ...
    for ad in ads:
        ...
        if re.search(r'\bОтделка помещений\b', name) or re.search(r'\bотделка помещений\b', name):
            hrefs_data.append(hrefs)
            for hre in hrefs_data:
                try:
                    session = requests.session()
                    r = session.get(hre, headers=headers)
...
def main():
    ...
    numb = [1, 10645, 10650, 10658, 10672, 10687, 10693, 10699, 10705, 10712, 10772, 10776, 10795, 10802, 10819, 10832,
            10841, 10842, 10176, 10853, 10857, 10933, 10939, 10939, 10174, 10897, 10904, 10926, 11004, 10946, 10950,
            11015,
            10995, 977, 11029, 11111, 11070, 11077, 11117, 11079, 11084, 11095, 11108, 11131, 11146, 11119, 11148,
            11153, 11156, 11235, 10231, 11330, 21949, 11266, 11282, 11309, 11316, 11318, 11353, 10233, 11340, 11158,
            11162, 11176, 11232, 11193, 11225, 11375, 10243, 11398,
            11403, 11409, 11443, 11450, 11457, 10251, 11010, 11012, 11013, 11020, 11021, 11069, 11024
            ]

    for n in numb:
        url = f'http://xmlriver.com/search_yandex/xml?user=6019&key=10e92a3587615ff8137a89723928af6c1c4ef396&query=отделка%20помещений&page=0&lr={n}'
